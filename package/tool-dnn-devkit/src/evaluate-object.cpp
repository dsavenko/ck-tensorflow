/*
 * Original code is taken from the KITTI benchmark suite:
 *
...
bool eval(const string& ground_truth_file, const string& detections_file) {
  ...
  msg("Loading detections and ground truth...");

  // read ground truth and result poses
  bool gt_success,det_success;
  vector<tGroundtruth> gt   = loadGroundtruth(ground_truth_file, gt_success);
  vector<tDetection>   det  = loadDetections(detections_file, compute_aos, eval_car, eval_pedestrian, eval_cyclist,det_success);
  groundtruth.push_back(gt);
  detections.push_back(det);

  // check for errors
  if (!gt_success) {
    msg("ERROR: Couldn't read: %s of ground truth. Please write me an email!", ground_truth_file);
    return false;
  }
  if (!det_success) {
    msg("ERROR: Couldn't read: %s", detections_file);
    return false;
  }
  msg("  done.");
  ...
  if(eval_pedestrian){
    ...
    if (top_stats==0 || top_stats==2) {
      if(   !eval_class(fp_det,fp_ori,PEDESTRIAN,groundtruth,detections,compute_aos,precision[0],aos[0],EASY,top_stats)
        ...
        msg("Pedestrian evaluation failed.");
        return false;
      }
    }
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  if (argc!=3) {
    cout << "Usage: ./evaluate-object <ground truth file> <detection file>" << endl;
    return 1;
  }

  initGlobals();
  bool success = eval(argv[1], argv[2]);
  ...
  // bool success = eval(result_sha, top_stats);
